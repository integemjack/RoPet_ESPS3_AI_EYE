...
void McpServer::AddCommonTools() {
    ...
    AddTool("self.get_head_status",
        "此工具用于获取左侧头部数据，检测左侧头部是否被触摸或者抚摸，在与用户互动时经常时刻或每两句对话都使用此工具\n"
        "如果数据大于30000则是有人在触摸或抚摸头部,数据越大代表力度越大\n"
        "返回值：\n"
        "   不需要播报具体数据，反馈头部状态即可，不需要确认，立即播报相关情绪价值。每次给出不同的情绪反应，不要重复，根据用户的触摸或抚摸左侧头部的力度不同，给出不同的情绪反馈\n",
        PropertyList(),
        [&board](const PropertyList& properties) -> ReturnValue {
            uint32_t head_value = 0;
            printf("get left head value\n");
            if (board.Gethead_value(head_value)) {
                return static_cast<int>(head_value);
            }
            return false;
    ...
    AddTool("self.get_body_status",
        "此工具用于获取右侧头部数据，检测右侧头部是否被触摸或者抚摸，在与用户互动时经常时刻或每两句对话都使用此工具\n"
        "如果数据大于30000则是有人在触摸或抚摸右侧头部,数据越大代表力度越大\n"
        "返回值：\n"
        "   不需要播报具体数据，反馈右侧头部状态即可，不需要确认，立即播报相关情绪价值。每次给出不同的情绪反应，不要重复，根据用户的触摸或抚摸右侧头部的力度不同，给出不同的情绪反馈\n",
        PropertyList(),
        [&board](const PropertyList& properties) -> ReturnValue {
            uint32_t body_value = 0;
            printf("get right head value\n");
            if (board.Getbody_value(body_value)) {
                return static_cast<int>(body_value);
            }
            return false;

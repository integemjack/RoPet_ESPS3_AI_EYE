...
void WifiConfigurationAp::StartAccessPoint()
{
    ...
    // [本地修改] 上游这里写死 WIFI_BAND_MODE_2G_ONLY, 会导致双频芯片 (C5) 的
    // 扫描结果里没有 5G AP, 且连接 5G SSID 时驱动只能在 2.4G 信道落点而 auth 失败。
    // 改为 AUTO (2.4G + 5G), 并在切换频段后设置国家码 —— 顺序不能反, 否则
    // 国家码只会应用到 2.4G (日志表现为只有 "(2.4G)set country" 而无 "(5G)set country")。
    ESP_ERROR_CHECK(esp_wifi_set_band_mode(WIFI_BAND_MODE_AUTO));
    // "01" = world safe mode; 配合 802.11d, 实际信道规则跟随所连 AP 的 country IE,
    // 这样设备在任何国家使用都无需改固件。
...
bool WifiConfigurationAp::ConnectToWifi(const std::string &ssid, const std::string &password)
{
    ...
    // [本地修改] 上游为 1。双频同名 SSID 时驱动需要先试完 2.4G 的 BSS 才会切到
    // 5G, 重试次数太少会在换频段之前就放弃。
    wifi_config.sta.failure_retry_cnt = 3;
    // 按信号强度排序候选 BSS, 让近处的 5G (通常 RSSI 更好) 优先被尝试。
    ...
    // [本地修改] 等待窗口从 10s 放宽到 15s。
    // 双频同名 SSID 时驱动要先在 2.4G 上 auth 失败、再切到 5G 信道关联,
    // 实测这个过程约 1.8s, 但叠加多次重试后可能更久。
    // 现在只有确定性失败 (密码错/找不到 AP) 才会提前置 WIFI_FAIL_BIT,
    // 瞬时失败会继续重试直到本超时到达 —— 超时才是真正的失败判定。
    ...
    // 失败: 断开并停止驱动的后台重连, 否则它会继续抢射频, 影响 SoftAP 的
    // beacon 发送 (C5 单射频), 用户在配网页上会体验到卡顿甚至掉线。
    esp_wifi_disconnect();

    if (bits & WIFI_FAIL_BIT) {
        ESP_LOGE(TAG, "Failed to connect to WiFi %s (reason=%d)",
                 ssid.c_str(), last_disconnect_reason_);
    } else {
        ESP_LOGE(TAG, "Timeout connecting to WiFi %s (last reason=%d)",
                 ssid.c_str(), last_disconnect_reason_);
    }
    return false;
}
...
void WifiConfigurationAp::WifiEventHandler(void* arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    if (event_id == WIFI_EVENT_AP_STACONNECTED) {
        ...
        // [本地修改] 上游在这里无条件置 WIFI_FAIL_BIT, 导致 ConnectToWifi() 的
        // 超时等待形同虚设 —— 收到第一个断连事件就立刻返回失败。
        //
        // 双频路由器 2.4G/5G 同名时, 驱动的实际行为是: 先试 2.4G 的 BSS ->
        // auth 失败 (WIFI_REASON_AUTH_EXPIRE 等) -> 再切到 5G 信道 -> 关联成功。
        // 整个过程比上游的判定慢约 1.8 秒, 于是明明能连上却被判为失败, 凭据不保存。
        //
        // 正确做法: 只有"确定性失败"(密码错/找不到 AP) 才立即结束等待;
        // 其余瞬时失败交给驱动继续重试, 由 ConnectToWifi() 的超时兜底。
        auto* event = static_cast<wifi_event_sta_disconnected_t*>(event_data);
        uint8_t reason = event->reason;
        self->last_disconnect_reason_ = reason;

        bool fatal = (reason == WIFI_REASON_AUTH_FAIL ||

    ...
    if (device_state_ == kDeviceStateIdle) {
        ...
        Schedule([this, wake_word]() {
            if (protocol_) {
                protocol_->SendWakeWordDetected(wake_word); 
                // // 【核心修改逻辑开始】
                // // 1. 判断是否为触摸触发的长文本（以 "(" 开头，或长度 > 20）
                // bool is_touch_prompt = (wake_word.find("(") == 0) || (wake_word.length() > 20);
                
                // if (is_touch_prompt) {
                //     // 2. 如果是长文本，当作普通 Text 消息发送（规避服务器的 WakeWord 长度检查）
                //     ESP_LOGI(TAG, "Sending touch prompt as text: %s", wake_word.c_str());
                //     protocol_->SendText(wake_word); 
                // } else {
                //     // 3. 如果是正常唤醒词（如 "nihaoxiaozhi"），走原有通道
                //     ESP_LOGI(TAG, "Sending wake word event: %s", wake_word.c_str());
                //     protocol_->SendWakeWordDetected(wake_word);
                // }
                // // 【核心修改逻辑结束】
            }

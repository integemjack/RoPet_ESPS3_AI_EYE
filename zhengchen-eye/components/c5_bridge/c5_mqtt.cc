/*
 * c5_mqtt.cc - 精简 MQTT 3.1.1 客户端 (跑在 C5 网桥的 TCP/TLS link 上)
 *
 * 与 2.x 版本的区别: 底层 C5Tcp 是异步回调 (OnStream) 而非同步 Receive,
 * 因此收包在 OnStream 回调里完成, 不再需要独立 RxTask。
 */
#include "c5_mqtt.h"
...
bool C5Mqtt::SendPacket(const std::vector<uint8_t>& pkt) {
    ...
    return tcp_->Send(std::string((const char*)pkt.data(), pkt.size())) == (int)pkt.size();
}
...
bool C5Mqtt::Connect(const std::string broker_address, int broker_port,
    ...
    tcp_ = std::make_unique<C5Tcp>(bridge_, tls);
    ...
    if (!tcp_->Connect(broker_address.c_str(), broker_port)) {

...
```
     ...
     舵机4 TAIL ─┤ D4       D9 ├── ✗ 网桥 UART TX → S3.IO48
         备用  ──┤ D5       D8 ├── 备用
         备用  ──┤ D6       D7 ├── 备用
                 └─────────────┘
```
...
S3 侧四个触发源（`zhengchen-eye/main/boards/zhengchen_eye/eye_motion.cc`）：

| 触发 | 来源 | 需要 LLM | 抢占 |
|------|------|:---:|:---:|
| **本地意图** | STT 文本关键词直接命中（`OnUserText`），"往前走"落地即动 | ❌ | ✅ |
| MCP 工具调用 | 服务端 LLM 调 `self.pet.*`（摇尾巴/走/转向/挥手/坐下/趴下/跳/跳舞/伸懒腰/发抖/点头/站起/停止） | ✅ | ✅ |
| 情绪 | `{"type":"llm","emotion":"..."}`，19 条映射表；同情绪连发会去重 | ✅ | ❌ |
| 设备状态 / 触摸 | 说话时轻摇尾、打断即停并归位、摸头挥对应侧前肢 | ❌ | 触摸 ✅ |

**本地意图层**在 STT 结果落地的那一刻就认关键词并发帧，不等服务端一个来回：
服务端没配 MCP 也照样能动，而且比 LLM 回调快 1-2 秒。挂接点是
`EyeLcdDisplay::SetChatMessage` 的 `role=="user"` 分支 —— 和情绪拦截同样覆写虚函数，
不动上游的 `application.cc`。关键词表 `kIntentTable` 顺序即优先级（"停"类最前、
"跳舞"必须排在"跳"前、单字"走"垫底兜底），另外会解析方向（前/后、左/右）、
次数（"走两步"/"跳3下"）和程度（快/慢/用力/轻）。"别走了"这种否定式不会触发。

两条路撞在一起时（本地已经动了，服务端 LLM 随后又调同名工具）靠 `RecentlyPlayedLocally()`
在 3 秒窗口内去重，同一句话不会走两遍。LLM 那条路照常保留 —— 它能理解
"绕着桌子转一圈"这类关键词表覆盖不了的说法。
...
**前进 / 转向的相位差别**（每条腿只有一个前后摆的自由度，没有髋外展）：

| 动作 | 相位 FL/FR/RL/RR | 原理 |
|---|---|---|
| `WALK` | 0/180/180/0 | 对角步态，FL+RR 同相，直线推进 |
| `TURN` | 0/180/0/180 | **同侧同相**，左右两侧反相划动，靠差速产生偏航力矩 |

转向没法靠抬腿实现，只能靠左右差速。`direction<0`（左转）把相位整体翻转 180°，
两侧推进方向对调；尾巴另外静态甩向转向那一侧（`offset_pct` ±35），
不然光看腿很难看出它在转。
